...
from layers import DSE, SpatialAttention, gnconv, ChannelAttention, IRMLP, firconv
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0,
                 in_chans=3, embed_dim=768, norm_layer=None, group=1):
        super().__init__()
...
class Pooling(nn.Module):
    def __init__(self, in_channels) -> None:
        ...
        # self.cmlp = CMlp(self.in_channels)
        self.cmlp = DSE(self.in_channels)
        self.spatten = SpatialAttention()
        self.gn = gnconv(self.in_channels)
        self.chatten = ChannelAttention(self.in_channels)
        self.ffn = IRMLP(self.in_channels * 2, self.in_channels)
...
class PoolFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        self.first = firconv(in_chans=3,embed_dim=embed_dims[0])
        self.patch_embed = PatchEmbed(
            patch_size=in_patch_size, stride=in_stride, padding=in_pad,
            in_chans=embed_dims[0], embed_dim=embed_dims[0],group=embed_dims[0])
...
flops, params = profile(net, inputs=(input_profile,))  # 计算量和参数量
print(flops)
print(params)
print(str(flops / (1024 ** 2))[:9], str(params / (1024 ** 2))[:9])
